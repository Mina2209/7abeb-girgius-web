...
export function VariousSection() {
  ...
  const deleteFile = (catId: string, fileId: string) => {
    setCategories(
      categories.map((cat: any) =>
        cat.id === catId
          ? { ...cat, files: cat.files.filter((f: any) => f.id !== fileId) }
          : cat,
      ),
    );
  };
    ...
    setCategories(
      categories.map((cat: any) => {
        ...
        if (newIndex >= 0 && newIndex < newFiles.length) {
          [newFiles[fileIndex], newFiles[newIndex]] = [
            newFiles[newIndex],
            newFiles[fileIndex],
          ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-4 mb-2">
            <h1 className="font-bold text-[36px] text-primary">
              بوربوينت متنوعة
            </h1>
            {isAuthorized && (
              <button
                  ...
                  {editingId === category.id ? (
                    <div className="flex items-center gap-2 flex-1 py-4">
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter")
                            updateCategoryTitle(category.id, tempName);
                      ...
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        {isEditMode && (
                          <div className="flex flex-col gap-0.5 shrink-0">
                            <button
                              onClick={() =>
                                moveFile(category.id, fileIndex, "up")
                              }
                            ...
                            <button
                              onClick={() =>
                                moveFile(category.id, fileIndex, "down")
                              }
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            {isEditMode && (
                              <div className="flex items-center gap-1 opacity-0 group-hover/file:opacity-100 transition-all">
                                ...
                                <button
                                  onClick={() =>
                                    updateFileSource(category.id, file.id)
                                  }
                                  className="p-1 text-muted-foreground hover:text-primary"
